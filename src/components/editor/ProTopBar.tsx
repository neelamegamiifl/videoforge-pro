...
interface P { dark:boolean; onToggleTheme:()=>void; onImportVideo:()=>void; onImportAudio:()=>void; onRecord:()=>void; recording:boolean; onDownloadVideo:()=>void; onDownloadMusic:()=>void; onAI:()=>void; onExport:()=>void; onGrade:()=>void; onMixer:()=>void; notify:(m:string,t?:any)=>void; }

export default function ProTopBar({ dark,onToggleTheme,onImportVideo,onImportAudio,onRecord,recording,onDownloadVideo,onDownloadMusic,onAI,onExport,onGrade,onMixer,notify }:P) {
  ...
  const btn = (accent=false, color='#e8375a') => ({ display:'flex',alignItems:'center',gap:5, background:accent?`${color}20`:c, border:`1px solid ${accent?color:bd}`, borderRadius:6, padding:'5px 10px', cursor:'pointer', color:accent?color:t, fontSize:11, fontWeight:600, whiteSpace:'nowrap' as const, transition:'all .15s' });
          ...
          <div style={{ position:'absolute', top:'100%', right:0, marginTop:3, background:'#111118', border:`1px solid ${bd}`, borderRadius:10, zIndex:200, minWidth:160, boxShadow:'0 12px 40px rgba(0,0,0,.6)', overflow:'hidden' }}>
            {session
              ? <><div style={{padding:'9px 14px',fontSize:11,color:m,borderBottom:`1px solid ${bd}`}}>{session.user?.email}</div>
                  <button onClick={()=>store.newProject()} style={{width:'100%',padding:'9px 14px',background:'none',border:'none',cursor:'pointer',color:t,fontSize:12,textAlign:'left' as const}}>📄 New Project</button>
