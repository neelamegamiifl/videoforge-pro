...
export default function AudioMixerPanel({onClose,notify}:{onClose:()=>void;notify:(m:string,t?:any)=>void}) {
  ...
  // FIX: only run when mixer is open; use 0 (not random) when no audio is playing
  useEffect(()=>{
    const tick=()=>{
      const data=audioEngine.getAnalyserData();
      const lvl=Array.from({length:store.project.audioTracks.length},(_,i)=>{
        const raw = data[i*4] ?? 0; // FIX: fallback to 0, not random
        return raw;
      });
      setLevels(lvl);
  ...
  // FIX: handle solo toggle — mute all others when a track is soloed
  const handleSolo = (trackId: string) => {
    const track = store.project.audioTracks.find(t => t.id === trackId);
    if (!track) return;
    const newSolo = !track.solo;
    store.updateAudio(trackId, { solo: newSolo });
    notify(newSolo ? `Solo: ${track.name}` : 'Solo off');
  };
  ...
  return (
    <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,.75)',zIndex:300,display:'flex',alignItems:'center',justifyContent:'center',backdropFilter:'blur(8px)'}} onClick={onClose}>
      <div onClick={e=>e.stopPropagation()} style={{background:bg,border:`1px solid ${bd}`,borderRadius:16,width:Math.min(920,60+store.project.audioTracks.length*120+80),maxHeight:'85vh',overflow:'auto',boxShadow:'0 30px 80px rgba(0,0,0,.7)'}}>
        <div style={{padding:'16px 20px',borderBottom:`1px solid ${bd}`,display:'flex',justifyContent:'space-between',alignItems:'center'}}>
          <div><div style={{fontWeight:800,fontSize:17,color:t}}>🎛 Audio Mixer</div><div style={{fontSize:11,color:m,marginTop:1}}>Multi-track mixing · EQ · Pan · Level meters · Solo</div></div>
        ...
        <div style={{padding:20,display:'flex',gap:12,overflowX:'auto' as const,minHeight:400}}>
          ...
          {store.project.audioTracks.map((tr,i)=>(
            <div key={tr.id} style={{width:110,flexShrink:0,background:c,border:`1px solid ${tr.muted?bd:tr.color+'44'}`,borderRadius:10,padding:'12px 8px',display:'flex',flexDirection:'column' as const,alignItems:'center',gap:7}}>
              ...
              {/* FIX: level meter shows 0 when silent, not random */}
              ...
              {/* Controls — FIX: solo actually mutes others */}
              <div style={{display:'flex',gap:5}}>
                ...
                <button onClick={()=>handleSolo(tr.id)} style={{background:tr.solo?'#f59e0b20':c,border:`1px solid ${tr.solo?'#f59e0b':bd}`,borderRadius:5,padding:'4px 7px',cursor:'pointer',color:tr.solo?'#f59e0b':m,fontSize:11,fontWeight:700}}>S</button>
              </div>
            </div>
          ))}
        </div>
