...
const SFX=[{n:'Whoosh',e:'💨'},{n:'Pop',e:'🎈'},{n:'Boom',e:'💥'},{n:'Ding',e:'🔔'},{n:'Clap',e:'👏'},{n:'Laugh',e:'😂'},{n:'Cash',e:'💰'},{n:'Swipe',e:'👆'},{n:'Drum',e:'🥁'},{n:'Alert',e:'🚨'},{n:'Rizz',e:'🔥'},{n:'Fail',e:'😬'}];
...
export default function ProLeftPanel({onImportVideo,onImportAudio,onDownloadMusic,notify}:P) {
  ...
  const addText=()=>{
    ...
    store.addText({text:txt,start:store.currentTime,duration:3,x:50,y:75,fontSize:fs,fontFamily:ff,color:fc,bgColor:bg?'rgba(0,0,0,0.7)':null,outlineColor:null,bold,italic,underline:false,align:'center',animation:anim==='None'?null:anim,animationOut:null,rotation:0,letterSpacing:0,shadow});
    setTxt('');notify('Text overlay added ✓');
  };
  ...
  return (
    <div style={{width:226,background:'#0d0d12',borderRight:`1px solid ${bd}`,display:'flex',flexDirection:'column',flexShrink:0}}>
      ...
      <div style={{flex:1,overflow:'auto',padding:11}}>
        ...
        {/* MEDIA */}
        {tab==='media'&&<div>
          <div onClick={onImportVideo} onDragOver={e=>{e.preventDefault();(e.currentTarget as HTMLDivElement).style.borderColor='#e8375a';}} onDragLeave={e=>{(e.currentTarget as HTMLDivElement).style.borderColor=bd;}}
          ...
          {store.project.clips.map(clip=>(
            <div key={clip.id} onClick={()=>store.setSelectedId(clip.id==='id'?null:clip.id,'clip')} onDoubleClick={()=>store.setSelectedId(clip.id,'clip')}
              ...
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                ...
                <div style={{display:'flex',gap:3,flexShrink:0}}>
                  ...
                  <button onClick={e=>{e.stopPropagation();store.deleteClip(clip.id);notify('Deleted');}} title="Delete" style={{background:'none',border:'none',cursor:'pointer',color:'#e8375a',fontSize:13,padding:2}}>✕</button>
                </div>
              </div>
              <div style={{height:2,borderRadius:1,background:clip.color,marginTop:7,opacity:.7}}/>

              {store.selectedId===clip.id&&(
                <div style={{marginTop:9,borderTop:`1px solid ${bd}`,paddingTop:9,animation:'slideUp .15s ease'}}>
                  ...
                  <div style={{display:'flex',gap:5,marginTop:6,flexWrap:'wrap' as const}}>
                    {[{l:'✂ Split',fn:()=>{store.splitClip(clip.id,store.currentTime);notify('Split ✂');}},{l:clip.muted?'🔊 Unmute':'🔇 Mute',fn:()=>store.updateClip(clip.id,{muted:!clip.muted})},{l:clip.locked?'🔓':'🔒',fn:()=>store.updateClip(clip.id,{locked:!clip.locked})},{l:'↩ H',fn:()=>store.updateClip(clip.id,{flipH:!clip.flipH})},{l:'↕ V',fn:()=>store.updateClip(clip.id,{flipV:!clip.flipV})}].map(b=>(
        ...
        {tab==='audio'&&<div>
          ...
          <div style={{fontSize:10,fontWeight:700,color:m,marginBottom:7,letterSpacing:.5}}>SOUND EFFECTS</div>
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:5,marginBottom:12}}>
            {SFX.map(s=>(
              <button key={s.n} onClick={()=>{store.addAudio({name:`${s.n} SFX`,url:null,type:'sfx',duration:1,volume:80,pan:0,fadeIn:0,fadeOut:0,loop:false,muted:false,solo:false,eq:{low:0,mid:0,high:0},trackIndex:1});notify(`${s.e} ${s.n}`);}} style={{background:c,border:`1px solid ${bd}`,borderRadius:7,padding:'7px 4px',cursor:'pointer',color:t,fontSize:11,textAlign:'center' as const,display:'flex',flexDirection:'column' as const,alignItems:'center',gap:2}}>
            ...
            <div key={tr.id} style={{background:c,border:`1px solid ${bd}`,borderRadius:8,padding:'10px',marginBottom:8}}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:7}}>
                ...
                <div style={{display:'flex',gap:3}}>
                  ...
                  <button onClick={()=>store.deleteAudio(tr.id)} style={{background:'none',border:'none',cursor:'pointer',color:'#e8375a',fontSize:12,padding:2}}>✕</button>
                </div>
              </div>
        ...
        {tab==='text'&&<div>
          ...
          <div style={{display:'flex',gap:5,marginBottom:10}}>
            {[{l:'B',v:bold,s:setBold,fw:700},{l:'I',v:italic,s:setItalic,fi:'italic'},{l:'BG',v:bg,s:setBg},{l:'Shadow',v:shadow,s:setShadow}].map(b=>(
              <button key={b.l} onClick={()=>b.s(!b.v)} style={{flex:1,background:b.v?'#e8375a15':c,border:`1px solid ${b.v?'#e8375a':bd}`,borderRadius:6,padding:'6px 3px',color:b.v?'#e8375a':m,cursor:'pointer',fontWeight:b.fw||400,fontStyle:(b as any).fi||'normal',fontSize:11}}>{b.l}</button>
            ))}
          </div>
          ...
          {store.project.textOverlays.map(ov=>(
            <div key={ov.id} style={{display:'flex',justifyContent:'space-between',alignItems:'center',background:c,border:`1px solid ${bd}`,borderRadius:7,padding:'7px 9px',marginBottom:5}}>
              <span style={{fontSize:12,color:ov.color,fontWeight:ov.bold?700:400,maxWidth:140,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{ov.text}</span>
        ...
        {tab==='fx'&&<div>
          ...
          {store.selectedId&&(()=>{
            ...
            return <div style={{marginTop:12}}><div style={{fontSize:10,fontWeight:700,color:m,marginBottom:7}}>APPLIED</div>
